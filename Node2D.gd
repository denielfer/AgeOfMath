...
func _process(delta):
	if start:
		...
		if base_inimiga.life <= 0:
			...
			bg.color= Color(0,0,0,.7)
			bg.size = Vector2(get_viewport_rect().size.x*1.1, get_viewport_rect().size.y*1.1)
			add_child(bg)
			var label = Label.new()
			label.text = win_text[level-1]
			bg.add_child(label)
			label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
			label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
			var btn = Button.new()
			btn.position = Vector2(get_viewport_rect().size.x/2, get_viewport_rect().size.y*.9)
			btn.text= 'Selecionar \n Level'
			add_child(btn)
